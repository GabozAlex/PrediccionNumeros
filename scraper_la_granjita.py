...
URL_BASE = "https://www.loteriadehoy.com/animalito/lagranjita/resultados/"
PREFIX = "La Granjita "
HEADERS = {
...
def setup_logging():
    if not os.path.exists('logs'):
        os.makedirs('logs')
    logger = logging.getLogger('la_granjita_scraper')
    ...
    file_handler = RotatingFileHandler(
        'logs/la_granjita_scraper.log', maxBytes=10*1024*1024, backupCount=5
    )
...
def save_to_excel(df, filename="data/LaGranjita.xlsx"):
    existing = None
...
if __name__ == "__main__":
    ...
    if option == "1":
        ...
        filename = input("Excel filename (default: data/LaGranjita.xlsx): ").strip() or "data/LaGranjita.xlsx"
        if os.path.exists(filename):
            existing = pd.read_excel(filename)
            existing['Fecha'] = pd.to_datetime(existing['Fecha']).dt.strftime("%Y-%m-%d")
